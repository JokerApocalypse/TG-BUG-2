  ...
  return (ctx, next) => {
    if (ctx.chat.type === "group" || ctx.chat.type === "supergroup") {
      if (allowedGroupIds.includes(ctx.chat.id)) {
        ...
        return ctx.reply("🚫 This group is lom in the head of the owner");
      }
    } else {
      return ctx.reply("❌ Special Group!");
    }
  };
};
...
const startSesi = async () => {
    ...
    if (usePairingCode && !cella.authState.creds.registered) {
        let phoneNumber = await question(chalk.black(chalk.bgCyan(`\Insert your whatsapp number without+\nExample 237656520674:\n`)));
    ...
    cella.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') {
            ...
            console.log(chalk.green('WhatsApp was successfully connected!'));
        }

        if (connection === 'close') {
            const shouldReconnect = lastDisconnect?.error?.output?.statusCode !== DisconnectReason.loggedOut;
            console.log(
                chalk.red('WhatsApp connection was cut off.'),
                shouldReconnect ? 'Try to reconnect ...' : 'Please re -login..'
            );
            if (shouldReconnect) {
                startSesi();
            }
...
╰━━━━━━━━━━━━━━━━⬣`;
    ...
    const keyboard = [
        [
            ...
            {
                text: "👁 Support Gp Owner",
                url: "https://t.me/+eGvR6oHOfa40YmE0"
            }
        ]
    ];
...
│ Successfuly send bug To ${target} ❤
│ © 𝐂𝐚𝐫𝐦𝐞𝐧𝐂𝐫𝐚𝐬𝐡💋
╰━━━━━━━━━━━━━━━━⬣`;
...
bot.command("brat", async (ctx) => {
    ...
    if (!text) {
        return ctx.reply("Enter text! Example: /brat Kerm");
    }
    ...
    try {
        ...
        console.error(error);
        ctx.reply("❌ An error occurred while making a sticker.");
    }
});
bot.command("gpt", async (ctx) => {
    ...
    if (!text) {
        return ctx.reply("Hi, what do I want to help you with? Enter text after command.");
    }
...
bot.command("play", async (ctx) => {
    ...
    if (!text) {
        return ctx.reply("Input keywords to search for YouTube videos! \n\nExample: /play Tiakola");
    }

    // Tampilkan reaksi pencarian
    await ctx.reply("🔎 Looking for a video...");
    ...
    try {
        ...
        if (!res) {
            return ctx.reply("❌ No results found for that keyword.");
        }
        ...
        if (anu.status) {
            ...
            await ctx.replyWithAudio(
                { url: urlMp3 },
                {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: "🔗 Watch on YouTube", url: res.url }],
                        ],
                    },
                }
            );
...
bot.command("ytmp3", async (ctx) => {
    ...
    if (!text.startsWith("https://")) {
        return ctx.reply("❌ Invalid link. Must start with 'https://'");
    }
...
bot.command("enc", async (ctx) => {
    ...
    if (!replyMessage || !replyMessage.document || !replyMessage.document.file_name.endsWith('.js')) {
        return ctx.reply('😠 Please reply to the .js file to be encrypted.');
    }
    ...
    ctx.reply("⚡️ Processing encrypt hard code . . .");
...
bot.command("xcbeta", checkWhatsAppConnection, async ctx => {
  ...
  if (!q) {
    return ctx.reply(`Example: command 237×××`);
  }
...
bot.command("xiosinvis", checkWhatsAppConnection, async ctx => {
  const q = ctx.message.text.split(" ")[1]; // Mengambil argumen pertama setelah perintah
    const userId = ctx.from.id;
...
bot.start(ctx => {
  const menuMessage = `
👋hello, welcome to CarmenCrash, press the button below to bring up the menu       

Hi ${greeting} ❤
╭━━━「 🌸WELCOME🌸 」━━━⬣
│ 
│ 𝐇𝐢💋💕
│ 𝐇𝐢 𝐁𝐚𝐛𝐲 𝐀𝐦 𝐇𝐞𝐫𝐞 𝐂𝐚𝐫𝐦𝐞𝐧𝐂𝐫𝐚𝐬𝐡💋
│       𝐋𝐨𝐯𝐞 𝐘𝐨𝐮 𝐛𝐚𝐛𝐲💋
├━━━「 📑 FEATURES 」━━━⬣
│ 
╰━━━━━━━━━━━━━━━━⬣
...
`;
  ...
  const photoUrl = "https://i.ibb.co/qdY2mx8/Manul-Ofc-X.jpg"; 


const keyboard = [
    [
        { text: "BUG-MENU", callback_data: "bugmenu" },
        { text: "☁️ Support Owner", url: "https://t.me/+eGvR6oHOfa40YmE0" }
    ],
    [
...
bot.action("ownermenu", (ctx) => {
  ...
  const menu = `
   👋 Hello baby, I am the 𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝘽𝙤𝙩  created by kerm. I'm here to assist you with anything you might need, making your interaction smoother and more efficient.       
Hi ${greeting}❤
ᝄ ⌜ 𝙊 𝙬 𝙣 𝙚 𝙧 𝙈 𝙚 𝙣 𝙪 ⌟
❤ > /delprem
❤ > /addprem
...
> © 𝐂𝐚𝐫𝐦𝐞𝐧𝐂𝐫𝐚𝐬𝐡💋
  ...
  const keyboard = [[{
    text: "Contact Owner",
    url: "https://t.me/Lord_Kerm"
  }]];

  ctx.replyWithPhoto("https://i.ibb.co/qdY2mx8/Manul-Ofc-X.jpg", {
    caption: menu,
    parse_mode: "Markdown",
    reply_markup: {
      inline_keyboard: keyboard
    ...
    ctx.replyWithAudio({
      url: "https://files.catbox.moe/2cn95t.mp3" 
    });
  });
});
bot.action("thaksto", (ctx) => {
  ...
  const greeting = new Date().getHours() < 12 ? "Pagi" : new Date().getHours() < 18 ? "Sore" : "Malam";
  const menu = `
   👋 Hello baby, this is a creadit from our sc kerm bug thanks to @Lord_Kerm because I used the base and sorry if this script is bad because it's only in the development stage.       
...
⟣──────────
  ...
  ctx.replyWithPhoto("https://i.ibb.co/qdY2mx8/Manul-Ofc-X.jpg", {
    ...
    ctx.replyWithAudio({
      url: "https://files.catbox.moe/40qb56.mp3" 
    });
  });
});
bot.action("menu", (ctx) => {
  ...
  const greeting = new Date().getHours() < 12 ? "Pagi" : new Date().getHours() < 18 ? "Sore" : "Malam";
  const menu = `
   👋 Hello baby, this is a creadit from our sc kerm bug thanks to @caywzz because I used the base and sorry if this script is bad because it's only in the development stage.       
...
> © 𝐂𝐚𝐫𝐦𝐞𝐧𝐂𝐫𝐚𝐬𝐡💋
  ...
  ctx.replyWithPhoto("https://i.ibb.co/qdY2mx8/Manul-Ofc-X.jpg", {
    ...
    ctx.replyWithAudio({
      url: "https://files.catbox.moe/cf6o7v.mp3" 
    });
  });
});
//Menu Awal
...
> © 𝐂𝐚𝐫𝐦𝐞𝐧𝐂𝐫𝐚𝐬𝐡💋
  ...
  ctx.replyWithPhoto("https://i.ibb.co/qdY2mx8/Manul-Ofc-X.jpg", {
    ...
    ctx.replyWithAudio({
      url: "https://files.catbox.moe/wabch9.mp3" 
    });
  });
});
...
> © 𝐂𝐚𝐫𝐦𝐞𝐧𝐂𝐫𝐚𝐬𝐡💋
  ...
  ctx.replyWithPhoto("https://i.ibb.co/qdY2mx8/Manul-Ofc-X.jpg", {
    ...
    ctx.replyWithAudio({
      url: "https://files.catbox.moe/iymbsw.mp3" 
    });
  });
});
bot.command("connect", async ctx => {
  ...
  ctx.reply("🔄 Connecting to whatsapp, please wait ...");
  try {
    await startSesi();
...
bot.command("status", ctx => {
  if (isWhatsAppConnected) {
    ctx.reply(`✅ WhatsApp is connected to the number: ${linkedWhatsAppNumber || "Don´t Worry Bro"}`);
  } else {
    ctx.reply("❌ WhatsApp has not been connected.");
  }
});
...
console.log(chalk.red.bold('Crash System Device By ☆ LordKerm'))
};
